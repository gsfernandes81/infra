...
| 2d | One base image for the four dev containers — `dev/README.md` § *Four copies of this* | `zero`'s dev containers | 2b | **All four converted 2026-08-24**; `2026.08.24.2` built and running on infra-dev and or3-dev. **`2026.08.25` supersedes it**: remote control is off the fleet (every container is ssh + abduco, the supervisors deleted) and the base now runs `offload-idle-claude.sh`, which stops a detached idle claude and leaves it resumable — one held 1,146 MB. Needs `gh workflow run dev-base.yml`, then a rebuild in all four repos |
