...
| 2d | One base image for the four dev containers — `dev/README.md` § *Four copies of this* | `zero`'s dev containers | 2b | **or3 merged 2026-08-24** (`cca66fd`), proving the base serves a child that is not infra-dev. **Not yet running:** `dev-base.yml` is dispatch-only, so merging built nothing — `2026.08.24.1` needs `gh workflow run`, then `make up` in both repos. `dd`/`ds` untouched |
