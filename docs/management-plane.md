...
| 2d | One base image for the four dev containers — `dev/README.md` § *Four copies of this* | `zero`'s dev containers | 2b | **`2026.08.24.1` built and running 2026-08-24** — infra-dev and or3-dev are both on it. **All four repos converted the same day**: dd and ds needed a fifth seam (`child-init.sh`, for the venv sync), so the base moved to **`2026.08.24.2`**, which is **not built yet** — `gh workflow run dev-base.yml`, then `make dev-up` in destiny-director and dossier, `make up` here when infra-dev is next rebuilt. or3-dev stays on `.1` until that repo moves its pin; nothing makes it |
