...
| The base carries the remote-control **hook**, never the daemon | Only an RC-enabled child uses it, dd-dev's is a different design, and `--permission-mode auto` is a decision about the container that makes it. In the base that default is one environment variable away in every child. |
| `remoteDialogSeen` is seeded **only** under `DEV_REMOTE_CONTROL=1` | It suppresses the one-time Remote Control consent prompt, which a headless container cannot answer. Seeding it where nothing ever starts remote-control is pre-consenting to a thing that container deliberately does not do. |
