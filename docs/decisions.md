...
| **One command writes every ssh block, and the composition is four static imports rather than a loop** | `configure-client.yml` imports `configure-client-fleet.yml` once and `configure-client-dev.yml` once per dev container, with the alias and port as import vars — the hostname is derived, see the row below. A loop over a list is the obvious spelling and is wrong here: `configure-client-dev.yml` prompts for a credential and writes it at 0600, and a loop puts those prompts and writes inside `loop:` results, where `no_log` hides which iteration is which — so a mistake becomes a mistake about a secret you cannot see. Static imports keep the reviewed single-alias play byte-for-byte as it was, each stanza reads as the fact it asserts, and `-e alias=…` still works for a single container, which is the rotation path. Verified: import vars reach the imported play, do not leak between imports, and `meta: end_play` inside one import does not stop the next. |
