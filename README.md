...
```sh
ssh -i ~/.ssh/id_ed25519_ddctl gavin@ssh-two.gsrpi.uk status
ssh -i ~/.ssh/id_ed25519_ddctl gavin@ssh-two.gsrpi.uk deploy-beacon
ssh -i ~/.ssh/id_ed25519_ddctl gavin@ssh-two.gsrpi.uk logs
```

That key is generated by `hosts/two/setup/root-setup.sh` on the box and pasted into the
Claude Code cloud environment block as `DD_CTL_KEY_B64`; a `SessionStart` hook in the
destiny-director repo writes it back out per session. Details, including why rotation
has to be asked for by name, are in
...
| `dd-ctl` dispatch key (private half) | wherever you deploy from, and the Claude Code cloud environment block as `DD_CTL_KEY_B64`. **That block is not masked** — which is why only this key, whose whole reach is dd-ctl's six verbs, belongs in it, and never one that gets a shell. `root-setup.sh` generates the pair on `two` and prints this half once; the public half stays in `~gavin/.ssh/authorized_keys` behind `command="/usr/local/bin/dd-ctl",restrict`. See [`hosts/two/system/README.md`](hosts/two/system/README.md) |
